                       ...
                       Bool_t debug)
{
  ...
  ht.addHist("dphivj0", 	  new TH1F("_dphivj0",          ";#Delta#phi(V,j0);Jets",20,0,4));
  ht.addHist("dphivj1", 	  new TH1F("_dphivj1",          ";#Delta#phi(V,j1);Jets",20,0,4));
  ht.addHist("dphivj2", 	  new TH1F("_dphivj2",          ";#Delta#phi(V,j2);Jets",20,0,4));
  ht.addHist("dphivj3", 	  new TH1F("_dphivj3",          ";#Delta#phi(V,j3);Jets",20,0,4));
      ...
      for( auto c : chTags) {
        ...
        for(size_t ij=0; ij<min(size_t(2),jets.size());ij++) {
          ...
          ht.fill("dr"+jtype+"b",   jets[ij].DeltaR(boson),  cplotwgts,c);
  	  	  ht.fill(c+"_jet_c1_00", 	ev.j_c1_00[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
          ht.fill(c+"_jet_c1_02", 	ev.j_c1_02[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
   	      ht.fill(c+"_jet_c1_05", 	ev.j_c1_05[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
	  	  ht.fill(c+"_jet_c2_00", 	ev.j_c2_00[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
      	  ht.fill(c+"_jet_c2_02", 	ev.j_c2_02[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
  	  	  ht.fill(c+"_jet_c2_05",	ev.j_c2_05[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
	  	  ht.fill(c+"_jet_c3_00", 	ev.j_c3_00[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
      	  ht.fill(c+"_jet_c3_02",	ev.j_c3_02[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
  	  	  ht.fill(c+"_jet_c3_05",	ev.j_c3_05[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
	  	  ht.fill(c+"_jet_zg", 		ev.j_zg[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
  	  	  ht.fill(c+"_jet_gaptd", 	ev.j_gaptd[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
  	  	  ht.fill(c+"_jet_gawidth", ev.j_gawidth[jets[ij].getJetIndex()]	  ,  cplotwgts,c);
          minEta=min(minEta,fabs(jets[ij].Eta()));
          maxEta=max(maxEta,fabs(jets[ij].Eta()));
        }
	    if(jets.size() > 2){
     		ht.fill(c+"_jjetas", 	  jets[0].Eta()*jets[1].Eta(),   cplotwgts,c);
        	ht.fill(c+"_dphivj0",  fabs(jets[0].DeltaPhi(boson)),  cplotwgts,c);
        	ht.fill(c+"_dphivj1",  fabs(jets[1].DeltaPhi(boson)),  cplotwgts,c);
        }
		if(jets.size() > 2){
          	ht.fill(c+"_dphivj2",  fabs(jets[2].DeltaPhi(boson)),  cplotwgts,c);
			int nCent = 0;
		  	for(unsigned int iJet = 2; iJet < jets.size(); iJet++){	
				...
				if(fabs(jets[ij].Rapidity() - sumy) < dy){
          			ht.fill(c+"_centjy", jets[ij].Rapidity(),  cplotwgts,c);
					nCent++;
				}
		  	}
		...
		if(jets.size() > 3)
         	ht.fill(c+"_dphivj3",  fabs(jets[3].DeltaPhi(boson)), cplotwgts,c);
