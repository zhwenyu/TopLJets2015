...
void VBFVectorBoson::RunVBFVectorBoson()
{
  ...
  reader->AddVariable("j_c2_05[0]",&leadj_c2_05);
  ...
  for (Int_t iev=0;iev<nentries;iev++)
	...
	if(isHighMJJ && photons[0].Pt()>minBosonHighPt)
	  cout<< "pt gamma = " << photons[0].Pt()<< ", mjj = " << mjj << "\t" << isVBF<<passMJJ<<isHighMJJ<<endl;
...
void VBFVectorBoson::addMVAvars(){
  ...
  newTree->Branch("leadj_c2_05", &leadj_c2_05);
  newTree->Branch("subleadj_c2_05", &subleadj_c2_05);
...
void VBFVectorBoson::initVariables(std::vector<Jet> jets){
  ...
  subleadj_c2_05   = (jets.size()>2 ? ev.j_c2_05[jets[1].getJetIndex()] : -99);
