...
def RunMethodPacked(args):
    ...
    hist,channel,boson,YEs,nBin,sigPH=args
    print args
    ...
    try:
        cmd='runWorkspaceProvider --Hist %s --Chan %s --V %s --YieldErr %s --nBin %d --doSignalPH' %(hist,channel,boson,YEs,nBin)
        if not sigPH:
            cmd='runWorkspaceProvider --Hist %s --Chan %s --V %s --YieldErr %s --nBin %d' %(hist,channel,boson,YEs,nBin)
        print(cmd)
        os.system(cmd)
    except :
        print 50*'<'
...
def main():
    ...
    parser.add_option(''  , '--Hist',        dest='hist',        help='variable name [%default]',                                         default='vbffisher',                    type='string')
    ...
    args = [opt.hist,opt.channel,opt.boson,opt.YEs,opt.nBin,opt.sigPH]
