...
class Plot(object):
    ...
    def show(self, outDir,lumi,noStack=False,saveTeX=False,extraText=None,noRatio=False,overlay=None):
        ...
        if not noRatio and self.dataH and len(self.mc)>0 :
            p2 = ROOT.TPad('p2','p2',0.0,0.0,1.0,0.2) # tpad -wz
            p2.Draw()
            ...
            ratioframe.GetYaxis().SetLabelSize(0.18)  # 0.18 -wz
            ratioframe.GetYaxis().SetTitleSize(0.2)  # 0.2
            ratioframe.GetYaxis().SetTitleOffset(0.3)
            ratioframe.GetXaxis().SetLabelSize(0.18)  # 0.18
            ratioframe.GetXaxis().SetTitleSize(0.2)   # 0.2
